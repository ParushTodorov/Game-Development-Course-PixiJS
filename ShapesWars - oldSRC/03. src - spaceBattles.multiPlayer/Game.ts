...
export class Game extends PIXI.Container {
    ...
    private update(delta: number) {
        this.gameObjects.forEach((gameObj) => {
            ...
            if (gameObj.shoot() && !this.playerOneShot && gameObj.getId() === 'PlayerOne') {
                this.createAmonition(gameObj);
                this.playerOneShot = true;
            };

            if (gameObj.shoot() && !this.playerTwoShot && gameObj.getId() === 'PlayerTwo') {
                this.createAmonition(gameObj);
                this.playerTwoShot = true;
            };


            if (gameObj.forDestroy() && this.playerOneShot && gameObj.getId() === 'AmoPlayerOne') {
                if (gameObj.killed()) {
                    this.gameObjects.forEach((gameobject) => {
                        this.deleteGameObj(gameobject.getId())
                    })
                    this.createText('Player One win');
                    return;
                };
            ...
            if (gameObj.forDestroy() && this.playerTwoShot && gameObj.getId() === 'AmoPlayerTwo') {
                if (gameObj.killed()) {
                    this.gameObjects.forEach((gameobject) => {
                        this.deleteGameObj(gameobject.getId())
                    })
                    this.createText('Player Two win');
                    return;
                };
                this.deleteGameObj(gameObj.getId());
